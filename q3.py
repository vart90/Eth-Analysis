...
if __name__ == "__main__":
    ...
    clean_lines=blocks.filter(c_b)
    bf = clean_lines.map(f_b)
    ...
    n = datetime.now() # current date and time
    datetime = now.strftime("%d-%m-%Y_%H:%M:%S")
    
    mrb = my_bucket_resource.Object(s3_bucket,'eth' + datetime + '/t10_miners.txt')
    
    mrb.put(Body=json.dumps(top10_min))
